...
export function ReviewExport({ state, ctrl }) {
  ...
  return h('div', null, [
    ...
    h('div',{ style:{ border:'1px solid #e5e7eb', borderRadius:'6px', padding:'16px 18px', marginBottom:'18px', background:'#fff'}},[
      ...
      h('div',{ style:{ marginTop:'4px'}}, selected.map((id,i)=>{ const m=poiMetaMap[id]||{ title:id, duration:0}; return h('div',{ key:id, style:{ display:'flex', alignItems:'center', gap:'10px', fontSize:'12px', padding:'8px 10px', border:'1px solid #e5e7eb', borderRadius:'6px', marginBottom:'6px', background:'#fafafa'}},[
